<template>
    <div class="ns-wechat-test-connection">
        <div class="border border-box-edge rounded p-4 bg-box-background">
            ...
            <p class="text-sm text-fontcolor-light mb-3">
                {{ __('Click the button below to test the WeChat Pay configuration and connection.') }}
            </p>
            <div class="flex items-center gap-2 flex-wrap">
                <button
                    @click="testConnection"
                    :disabled="testing"
                    class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                    <span v-if="testing">{{ __('Testing...') }}</span>
                    <span v-else>{{ __('Test WeChat Pay Connection') }}</span>
                </button>
...
export default {
    name: 'ns-wechat-test-connection',
    data() {
        return {
            testing: false,
...
<style scoped>
.ns-wechat-test-connection {
    width: 100%;
}

.ns-wechat-test-connection > div {
    min-height: auto;
}
</style>
