...
import { default as nsMaskedInput } from './ns-masked-input.vue';
import { default as nsMaskedTextarea } from './ns-masked-textarea.vue';
...
export default {
    ...
    computed: {
        ...
        isMaskedText() {
            return ['masked-text'].includes(this.field.type);
        },
        isMaskedTextarea() {
            return ['masked-textarea'].includes(this.field.type);
        },
        isCheckbox() {
...
<template>
    <template v-if="isHiddenField">
        <input type="hidden" :name="field.name" :value="field.value" v-bind="$attrs" />
    </template>
    <div class="flex flex-auto" :class="field.label ? 'mb-2': ''" v-if="!isHiddenField" v-bind="$attrs">
        ...
        <ns-media-input @blur="$emit('blur', field)" @change="changeTouchedState(field, $event)" :field="field"
            v-if="isMedia">
            <template v-slot>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-media-input>
        ...
        <ns-search-select :field="field" @saved="handleSaved(field, $event)" @change="changeTouchedState(field, $event)"
            v-if="isSearchField">
            <template v-slot>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-search-select>
        <ns-daterange-picker @blur="$emit('blur', field)" @change="changeTouchedState(field, $event)" :field="field"
        ...
        <ns-textarea @blur="$emit('blur', field)" @change="changeTouchedState(field, $event)" :field="field"
            v-if="isTextarea">
            <template>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-textarea>
        <ns-masked-input @blur="$emit('blur', field)" @change="changeTouchedState(field, $event)" :field="field"
            v-if="isMaskedText">
            <template>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-masked-input>
        <ns-masked-textarea @blur="$emit('blur', field)" @change="changeTouchedState(field, $event)" :field="field"
            v-if="isMaskedTextarea">
            <template>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-masked-textarea>
        <ns-checkbox @blur="$emit('blur', field)" @change="changeTouchedState(field, $event)" :field="field"
            v-if="isCheckbox">
            <template v-slot>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-checkbox>
        <ns-inline-multiselect @blur="$emit('blur', field)" @update="changeTouchedState(field, $event)" :field="field"
            v-if="isInlineMultiselect">
            <template v-slot>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-inline-multiselect>
        <ns-multiselect @addOption="addOption($event)" @removeOption="removeOption($event)" :field="field"
            v-if="isMultiselect">
            <template v-slot>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-multiselect>
        <ns-ckeditor :field="field" v-if="isCkEditor">
            <template v-slot>{{ field.label }}</template>
            <template v-slot:description><span v-html="field.description || ''"></span></template>
        </ns-ckeditor>
