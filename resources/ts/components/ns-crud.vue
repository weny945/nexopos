...
export default {
    ...
    methods: {
        __,
        /**
         * Get storage key for current CRUD instance
         * Uses src URL to create a unique key
         */
        getStorageKey() {
            // Create a simple hash from src URL for storage key
            const srcKey = this.src.replace( /[^a-zA-Z0-9]/g, '_' );
            return `ns-crud-state-${srcKey}`;
        },
        /**
         * Save current state to sessionStorage
         */
        saveState() {
            // Build columns state only if columns is properly initialized
            let columnsState = [];
            if ( this.columns && typeof this.columns === 'object' ) {
                columnsState = Object.keys( this.columns ).map( key => ( {
                    key,
                    $sorted: this.columns[ key ].$sorted,
                    $direction: this.columns[ key ].$direction,
                } ) );
            }
            ...
            const state = {
                ...
                queryFiltersString: this.queryFiltersString,
        ...
        restoreState() {
            try {
                ...
                if ( saved ) {
                    ...
                    this.queryFiltersString = state.queryFiltersString || '';
                    this.withFilters = state.withFilters || false;
                }
            } catch ( e ) {
                console.warn( 'Failed to restore CRUD state:', e );
            }
        },
        /**
         * Restore column sort state from saved state
         */
        restoreColumnSortState() {
            try {
                const saved = sessionStorage.getItem( this.getStorageKey() );
                if ( saved ) {
                    const state = JSON.parse( saved );
                    if ( state.columns && this.columns ) {
                        state.columns.forEach( savedCol => {
                            if ( this.columns[ savedCol.key ] ) {
                                this.columns[ savedCol.key ].$sorted = savedCol.$sorted;
                                this.columns[ savedCol.key ].$direction = savedCol.$direction;
                            }
                        } );
                    }
                }
            } catch ( e ) {
                console.warn( 'Failed to restore column sort state:', e );
            }
        },
        getQueryParams() {
            if ( this.queryParams ) {
        ...
        loadConfig() {
            ...
            request.subscribe( (f:any) => {
                ...
                // Restore column sort state after columns are loaded
                this.$nextTick( () => {
                    this.restoreColumnSortState();
                } );
