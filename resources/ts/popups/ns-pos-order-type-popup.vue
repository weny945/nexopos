...
export default {
    ...
    mounted() {
        ...
        } else if ( Object.values( this.types ).length > 1 ) {
            /**
             * If there are multiple order types, check if there's a default type
             * and auto-select it if no type is currently selected.
             */
            const hasSelectedType = Object.values( this.types ).some( type => type.selected );

            if ( ! hasSelectedType && this.urls.default_order_type ) {
                const defaultType = Object.keys( this.types ).find( key => key === this.urls.default_order_type );

                if ( defaultType ) {
                    this.select( defaultType );
                }
            }
        }
