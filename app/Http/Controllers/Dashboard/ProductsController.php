...
class ProductsController extends DashboardController
{
    ...
    /**
     * Convert product unit quantity to linked product.
     *
     * @param  Request  $request
     * @param  ProductUnitQuantity  $unitQuantity
     * @return array
     */
    public function convertToLinkedProduct( Request $request, ProductUnitQuantity $unitQuantity )
    {
        ns()->restrict( [ 'nexopos.convert.products-units' ] );
        ...
        return $this->productService->convertToLinkedProduct(
    ...
    public function searchUsingArgument( $reference )
    {
        ...
        if ( $product instanceof Product ) {
             ...
             * Priority: selectedUnitQuantity > first unit_quantity
             */
            $unitQuantity = $product->selectedUnitQuantity ?? $product->unit_quantities->first();

            if ( ! $unitQuantity ) {
                throw new NotFoundException( __( 'The product doesn\'t have any unit quantities defined.' ) );
            }

            // Load unit if not already loaded
            if ( ! $unitQuantity->relationLoaded( 'unit' ) ) {
