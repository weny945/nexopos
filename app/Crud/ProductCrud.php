...
class ProductCrud extends CrudService
{
    ...
    public function getForm( $entry = null )
    {
        ...
        /**
         * Add linked product selector field using hook
         * Note: Only visible for non-base units
         */
        $fields[] = [
            'type' => 'custom',
            'label' => __( 'Linked Base Unit Product' ),
            'name' => 'linked_product_id',
            'description' => __( 'Link this non-base unit to a base unit product for conversion. The conversion will use the unit value defined in the unit group (e.g., 1 box = 20 bags based on unit value).' ),
            'value' => '',
            'component' => 'nsLinkedProductSelector',
        ];
                        ...
                        'identification' => [
                            ...
                            'fields' => [
                                [
                                    ...
                                    'validation' => 'required',
                                    'value' => $entry->barcode_type ?? 'ean13',
                                ], [
                                    'type' => 'select',
                                        ...
                                        ->map( function ( $productUnitQuantity ) use ( $fields ) {
                                            ...
                                            return [
                                                'fields' => collect( $fields )->map( function ( $field ) use ( $productUnitQuantity ) {
                                                    ...
                                                    /**
                                                     * Add unit information to custom fields
                                                     * This allows components like nsLinkedProductSelector to check if it's a base unit
                                                     */
                                                    if ( $field[ 'type' ] === 'custom' ) {
                                                        $field[ 'unit' ] = [
                                                            'id' => $productUnitQuantity->unit_id,
                                                            'base_unit' => $productUnitQuantity->unit ? $productUnitQuantity->unit->base_unit : 0,
                                                        ];
                                                    }
