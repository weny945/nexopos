...
class ProductCategoryCrud extends CrudService
{
    ...
    public function bulkAction( Request $request )
    {
        ...
        /**
         * Compute products for selected categories
         */
        if ( $request->input( 'action' ) == 'compute_selected' ) {
            $status = [
                'success' => 0,
                'error' => 0,
            ];

            $categoryService = app()->make( ProductCategoryService::class );

            foreach ( $request->input( 'entries' ) as $id ) {
                $entity = $this->model::find( $id );
                if ( $entity instanceof ProductCategory ) {
                    $categoryService->computeProducts( $entity );
                    $status[ 'success' ]++;
                } else {
                    $status[ 'error' ]++;
                }
        ...
        /**
         * Compute products for all categories
         */
        if ( $request->input( 'action' ) == 'compute_all' ) {
            $status = [
                'success' => 0,
                'error' => 0,
            ];
    ...
    public function getBulkActions(): array
    {
        return Hook::filter( $this->namespace . '-bulk', [
            ...
            [
                'label' => __( 'Compute Products (Selected)' ),
                'identifier' => 'compute_selected',
                'url' => ns()->route( 'ns.api.crud-bulk-actions', [
                    'namespace' => $this->namespace,
                ] ),
            ],
            [
                'label' => __( 'Compute Products (All)' ),
                'identifier' => 'compute_all',
                'url' => ns()->route( 'ns.api.crud-bulk-actions', [
                    'namespace' => $this->namespace,
                ] ),
            ],
