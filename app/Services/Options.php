...
use App\Services\PaymentCodeSecurityService;
...
class Options
{
    ...
    public function setDefault( $options = [] ): void
    {
        ...
        $defaultOptions = [
            ...
            'ns_orders_default_type' => 'takeaway',
            'ns_payment_alipay_enabled' => 'no',
            'ns_payment_wechat_enabled' => 'no',
        ];
    ...
    public function encodeOptionValue( Option $option, mixed $value ): void
    {
        ...
        $sensitiveFields = [
            'ns_payment_alipay_private_key',
            'ns_payment_alipay_public_key',
            'ns_payment_wechat_api_key',
        ];
        ...
        if ( is_array( $value ) ) {
            ...
            if ( in_array( $option->key, $sensitiveFields ) ) {
                $option->value = PaymentCodeSecurityService::encrypt( $value );
            } else {
                $option->value = $value;
            }
        }
    ...
    public function decodeOptionValue( $option )
    {
        ...
        if ( ! empty( $option->value ) && $option->isClean() ) {
            if ( is_string( $option->value ) && $option->array ) {
                ...
                if ( in_array( $option->key, $sensitiveFields ) ) {
                    $option->value = PaymentCodeSecurityService::decrypt( $option->value );
                }
