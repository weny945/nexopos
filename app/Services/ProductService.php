...
class ProductService
{
    ...
    private function __computeUnitQuantities( $fields, $product )
    {
        if ( $fields[ 'units' ] ) {
            foreach ( $fields[ 'units' ][ 'selling_group' ] as $group ) {
                ...
                /**
                 * Handle linked product fields
                 * Validate: Only non-base units can link to other products
                 */
                $linkedProductId = $group[ 'linked_product_id' ] ?? null;

                if ( $linkedProductId ) {
                    // Check if this is a base unit
                    $unit = Unit::find( $group[ 'unit_id' ] );

                    if ( $unit && $unit->base_unit ) {
                        throw new NotAllowedException(
                            __( 'Base units cannot be linked to other products. Only non-base units can use this feature.' )
                        );
                    }

                    $unitQuantity->linked_product_id = $linkedProductId;
                } else {
                    $unitQuantity->linked_product_id = null;
                }
    ...
    /**
     * Convert product unit quantity to a linked product.
     *
     * @param  ProductUnitQuantity  $unitQuantity
     * @param  float  $quantity
     * @return array
     *
     ...
     */
    public function convertToLinkedProduct( ProductUnitQuantity $unitQuantity, float $quantity = 1 ): array
    {
        /**
         * Validation 1: Check if stock management is enabled
         */
        ...
        /**
         * Validation 2: Check if linked product is set
         */
        if ( ! $unitQuantity->linked_product_id ) {
            throw new NotAllowedException( __( 'This unit is not linked to any product.' ) );
        }
         ...
         */
        if ( $quantity <= 0 ) {
            throw new NotAllowedException( __( 'The quantity to convert must be greater than zero.' ) );
        }

        /**
         * Validation 4: Check if current unit has enough quantity
         */
        if ( $unitQuantity->quantity < $quantity ) {
            throw new NotAllowedException(
                sprintf(
                    __( 'Not enough quantity. Available: %s, Required: %s' ),
        ...
        /**
         * Validation 5: Check if linked product exists
         */
        $linkedProduct = Product::find( $unitQuantity->linked_product_id );

        if ( ! $linkedProduct instanceof Product ) {
            throw new NotFoundException( __( 'The linked product no longer exists.' ) );
        }

        /**
         * Validation 6: Check if linked product has visible unit quantities
         */
        $linkedUnitQuantities = ProductUnitQuantity::where( 'product_id', $linkedProduct->id )
            ->where( 'visible', true )
            ->get();

        if ( $linkedUnitQuantities->isEmpty() ) {
            throw new NotFoundException( __( 'The linked product has no visible unit quantities.' ) );
        }

        /**
         * Load the unit to get its value for conversion calculation
         * The unit.value defines how many base units equal 1 of this unit
         */
        $unitQuantity->load( 'unit' );
        $unitValue = $unitQuantity->unit->value ?? 1;

        /**
         * Calculate target quantity using unit value
         * Example: If unit.value = 20, then 1 box = 20 base units
         */
        $targetQuantity = $quantity * $unitValue;

        /**
         * Get the first visible unit quantity from linked product
         */
        $linkedUnitQuantity = $linkedUnitQuantities->first();

        /**
         * Perform the conversion using database transaction
         */
        return ns()->database->transaction( function () use ( $product, $unitQuantity, $quantity, $linkedProduct, $linkedUnitQuantity, $targetQuantity, $unitValue ) {
            /**
             * Step 1: Reduce quantity from current product unit
             */
            $this->handleStockAdjustmentRegularProducts(
                action: ProductHistory::ACTION_REMOVED,
                product_id: $product->id,
                ...
                total_price: ns()->currency->define( $unitQuantity->sale_price )->multipliedBy( $quantity )->toFloat(),
            );

            /**
             * Step 2: Add quantity to linked product unit
             */
            $this->handleStockAdjustmentRegularProducts(
                action: ProductHistory::ACTION_ADDED,
                product_id: $linkedProduct->id,
                unit_id: $linkedUnitQuantity->unit_id,
                unit_price: $linkedUnitQuantity->sale_price,
                quantity: $targetQuantity,
                procurementProduct: null,
                total_price: ns()->currency->define( $linkedUnitQuantity->sale_price )->multipliedBy( $targetQuantity )->toFloat(),
            );

            /**
             * Reload the unit quantity to get updated values
             */
            $unitQuantity->refresh();
            $unitQuantity->load( 'unit' );
            $linkedUnitQuantity->refresh();
            $linkedUnitQuantity->load( 'unit' );

            return [
                'status' => 'success',
                'message' => sprintf(
                    __( 'Successfully converted %s %s of %s to %s %s of %s' ),
                    $quantity,
                    $unitQuantity->unit->name,
                    $product->name,
                    $targetQuantity,
                    $linkedUnitQuantity->unit->name,
                    $linkedProduct->name
                ),
                'data' => [
                    'from_product' => $product->name,
                    'from_quantity' => (string) $quantity,
                    'from_unit' => $unitQuantity->unit->name,
                    'to_product' => $linkedProduct->name,
                    'to_quantity' => (string) $targetQuantity,
                    'to_unit' => $linkedUnitQuantity->unit->name,
                ],
            ];
        } );
    }
