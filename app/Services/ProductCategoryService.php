...
class ProductCategoryService
{
    ...
    public function computeProducts( ProductCategory $category )
    {
        ...
        // Recursively count products from all sub-categories
        $subCategoryProducts = 0;
        $subCategories = $category->subCategories;

        foreach ($subCategories as $subCategory) {
            // Recursively compute for each sub-category first
            $this->computeProducts($subCategory);
            // Then add their total_items to our count
            $subCategoryProducts += $subCategory->total_items;
        }

        // Total = direct products + all sub-category products (recursive)
        $category->total_items = $directProducts + $subCategoryProducts;
        $category->save();
    }
