...
class SetupService
{
    ...
    public function createDefaultPayment( $user )
    {
        ...
        $alipayPaymentType = new PaymentType;
        $alipayPaymentType->label = __( 'Alipay' );
        $alipayPaymentType->identifier = 'alipay-payment';
        $alipayPaymentType->active = false; // 默认禁用，需要配置后启用
        $alipayPaymentType->priority = 5;
        $alipayPaymentType->author = $user->id;
        $alipayPaymentType->save();

        $wechatPaymentType = new PaymentType;
        $wechatPaymentType->label = __( 'WeChat Pay' );
        $wechatPaymentType->identifier = 'wechat-payment';
        $wechatPaymentType->active = false; // 默认禁用，需要配置后启用
        $wechatPaymentType->priority = 6;
        $wechatPaymentType->author = $user->id;
        $wechatPaymentType->save();
    }
